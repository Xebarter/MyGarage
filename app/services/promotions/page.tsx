...
export default function ServicePromotionsPage() {
  ...
  useEffect(() => {
    ...
    if (!currentVendorId) {
      setLoading(false);
      setError('No active provider session. Please log in again.');
      return;
    }
  ...
  const loadData = async (currentVendorId: string) => {
    ...
    setError(null);
    try {
      const [promotionsRes, appsRes] = await Promise.all([
        fetch('/api/promotions'),
        fetch(`/api/ad-applications?vendorId=${currentVendorId}`),
      ]);

      if (!promotionsRes.ok || !appsRes.ok) {
        throw new Error('Failed to load promotions workspace');
      }

      const [promotionsData, appsData] = await Promise.all([promotionsRes.json(), appsRes.json()]);

      const localServices = localStorage.getItem(STORAGE_KEY);
      const parsedServices = localServices ? (JSON.parse(localServices) as ManagedService[]) : [];

      setPromotions(Array.isArray(promotionsData) ? promotionsData : []);
      setApplications(Array.isArray(appsData) ? appsData : []);
      setServices(parsedServices);
    } catch (loadError) {
      console.error('Failed to load service promotions workspace:', loadError);
  ...
  const submitApplication = async () => {
    ...
    if (scope === 'single' && !selectedService) {
      window.alert('Please select a service for single-service ad application.');
      return;
    }
    ...
    try {
      const response = await fetch('/api/ad-applications', {
        ...
        body: JSON.stringify({
          vendorId,
          scope,
          productId: scope === 'single' ? selectedService?.id : undefined,
          productName: scope === 'single' ? selectedService?.name : undefined,
      ...
      window.alert('Service ad application submitted successfully.');
    } catch (submitError) {
      console.error('Failed to submit service ad application:', submitError);
      window.alert('Unable to submit service ad application right now. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="space-y-8 p-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Discover active campaigns and manage your service ad applications in one place.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 justify-items-start gap-2.5 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Card className="h-[96px] w-full max-w-[240px] p-3">
          ...
          <p className="mt-1 text-xs text-muted-foreground">Submitted from your service provider account</p>
        </Card>
        <Card className="h-[96px] w-full max-w-[240px] p-3">
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-5">
        <Card className="space-y-4 p-5 xl:col-span-2">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold">New Service Ad Application</h2>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <Button type="button" variant={scope === 'single' ? 'default' : 'outline'} onClick={() => setScope('single')}>
                Single service
              </Button>
            </div>
          </div>
      ...
      <Card className="space-y-4 p-5">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold">My Service Ad Applications</h2>
                  ...
                  <div>
                    <p className="font-medium">
                      {application.scope === 'all'
                        ? 'All services ad request'
                        : `Single service ad request (${application.productName || application.productId})`}
                    </p>
                    <p className="mt-1 text-xs text-muted-foreground">
                      Request #{application.id} • Submitted {toDate(application.createdAt).toLocaleString()}
                    </p>
                    {application.message && <p className="mt-2 text-sm text-muted-foreground">{application.message}</p>}
                  </div>
