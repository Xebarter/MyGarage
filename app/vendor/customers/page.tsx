...
export default function VendorCustomersPage() {
  ...
  const customerRows = useMemo(() => {
    const map = new Map<string, CustomerRow & { productCounts: Record<string, number> }>();
    ...
    for (const order of orders) {
      ...
      if (order.shippingAddress && !row.shippingAddresses.includes(order.shippingAddress)) {
        row.shippingAddresses.push(order.shippingAddress);
      }

      for (const item of vendorItems) {
        const name = item.productName || 'Unnamed product';
        row.productCounts[name] = (row.productCounts[name] || 0) + item.quantity;
      }
    }

    return Array.from(map.values()).map((row) => {
      const topProducts = Object.entries(row.productCounts)
  ...
  const filteredCustomers = useMemo(() => {
    const withFilters = customerRows.filter((customer) => {
      ...
      const matchesSegment = (() => {
        ...
        if (segmentFilter === 'new') return customer.ordersCount === 1;
        if (segmentFilter === 'returning') return customer.ordersCount >= 2;
        return customer.ordersCount >= 5;
      })();
  ...
  const customerStats = useMemo(() => {
    ...
    const activeLast30Days = customerRows.filter((customer) => {
      return new Date(customer.lastOrderDate).getTime() >= thirtyDaysAgo;
    }).length;
    ...
    <div className="space-y-8 p-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Understand buyer behavior, identify repeat customers, and prioritize high-value relationships.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 justify-items-start gap-2.5 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Card className="h-[96px] w-full max-w-[240px] p-3">
          ...
          <p className="mt-1 text-[11px] text-muted-foreground">Placed 2+ orders</p>
        </Card>
        <Card className="h-[96px] w-full max-w-[240px] p-3">
          ...
          <p className="mt-1 text-[11px] text-muted-foreground">Placed 5+ orders</p>
        </Card>
        <Card className="h-[96px] w-full max-w-[240px] p-3">
          <p className="mb-1 text-xs uppercase tracking-wide text-muted-foreground">Customer Revenue</p>
          <p className="text-xl font-bold leading-none">UGX {customerStats.lifetimeRevenue.toFixed(0)}</p>
          <p className="mt-1 text-[11px] text-muted-foreground">Total vendor sales</p>
        </Card>
      </div>
        ...
        {error ? (
          ...
          <Card className="p-8 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Try adjusting your filters, or check back after new customer orders are placed.
            </p>
          </Card>
        ) : (
            ...
            <Card key={customer.id} className="p-6">
              <div className="mb-5 flex flex-col gap-3 border-b border-border pb-4 md:flex-row md:items-start md:justify-between">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {customer.activeStatuses > 0 && (
                    <Badge variant="outline">Active Orders: {customer.activeStatuses}</Badge>
                  )}
                </div>
              </div>
              ...
              <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="rounded-md border border-border p-3">
                  <p className="mb-2 text-sm font-medium">Top Purchased Products</p>
                  {customer.topProducts.length === 0 ? (
                    <p className="text-xs text-muted-foreground">No product purchase history yet.</p>
                  ) : (
                ...
                <div className="rounded-md border border-border p-3">
                  <p className="mb-2 text-sm font-medium">Shipping Addresses</p>
                  {customer.shippingAddresses.length === 0 ? (
                    <p className="text-xs text-muted-foreground">No shipping addresses found.</p>
                  ) : (
                    <div className="space-y-1.5">
                      {customer.shippingAddresses.slice(0, 2).map((address) => (
                        <p key={address} className="text-sm text-muted-foreground">{address}</p>
                      ))}
                      {customer.shippingAddresses.length > 2 && (
                        <p className="text-xs text-muted-foreground">
                          +{customer.shippingAddresses.length - 2} more saved addresses
                        </p>
                      )}
                    </div>
                  )}
