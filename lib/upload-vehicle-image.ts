export type UploadVehicleImageProgress = {
  loaded: number;
  total: number;
  percent: number;
};
...
export async function uploadVehicleImage(
  file: File,
  opts?: { signal?: AbortSignal; onProgress?: (p: UploadVehicleImageProgress) => void },
): Promise<string> {
  ...
  return await new Promise<string>((resolve, reject) => {
    ...
    xhr.open("POST", "/api/uploads/vehicle-image", true);
