export type UploadListingImageProgress = {
  loaded: number;
  total: number;
  percent: number;
};
...
export async function uploadListingImage(
  file: File,
  opts?: { signal?: AbortSignal; onProgress?: (p: UploadListingImageProgress) => void },
): Promise<string> {
  ...
  return await new Promise<string>((resolve, reject) => {
    ...
    xhr.open("POST", "/api/uploads/listing-image", true);
