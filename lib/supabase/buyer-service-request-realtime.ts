...
export function mergeRealtimeRowIntoRequestDetail<T extends Record<string, unknown>>(prev: T, row: Row): T {
  return {
    ...
    cancelledAt: pickStrNull(row, 'cancelled_at', (prev.cancelledAt as string | null) ?? null),
    cancelledBy: pickStrNull(row, 'cancelled_by', (prev.cancelledBy as string | null) ?? null),
    cancellationReason: pickStrNull(row, 'cancellation_reason', (prev.cancellationReason as string | null) ?? null),
