export type UploadVehicleDocumentProgress = {
  loaded: number;
  total: number;
  percent: number;
};
...
export async function uploadVehicleDocument(
  file: File,
  opts?: { signal?: AbortSignal; onProgress?: (p: UploadVehicleDocumentProgress) => void },
): Promise<string> {
  ...
  return await new Promise<string>((resolve, reject) => {
    ...
    xhr.open("POST", "/api/uploads/vehicle-document", true);
