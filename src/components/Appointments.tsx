...
export function Appointments({ statusFilter = 'all', dateFilter = '' }: AppointmentsProps) {
  ...
  // Update local date filter when prop changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Apply date filter
    if (localDateFilter) {
      result = result.filter(appointment => appointment.date === localDateFilter)
    }
  ...
  }, [appointments, statusFilter, statusFilters, localDateFilter, searchTerm, sortConfig])
        ...
        {currentAppointments.length > 0 ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {statusFilters.length > 0 || localDateFilter || searchTerm
